...
//
// The week's number is written at the top, because the same screen shows a
// week nobody has kicked a ball in yet and a week that finished on Sunday,
// and the difference between them is worth stating rather than inferring.
// Last week's results fold away at the foot of the list: the fantasy clock
// only turns the week over at the next deadline, and when it does, the
// football you have just watched should be one click away, not gone.
Item {
  id: tab
  ...
  readonly property string standing: {
    ...
    var live = st.live_matches || 0
    if (live > 0) return live + (live === 1 ? " match in play" : " matches in play")
    var started = 0, done = 0
    for (var i = 0; i < fx.length; i++) {
      if (fx[i].started) started++
      if (fx[i].finished) done++
    }
    if (done === fx.length) return "all " + fx.length + " matches finished"
    if (started === 0) return "first kick-off " + Fmt.kickoff(fx[0].kickoff)
    return (fx.length - done) + " still to come"
  }

  function matchRow(f, squad, races) {
    var mine = []
    ...
    return { kind: "match", fixture: f, mine: mine, race: race }
  }

  readonly property var pastRows: {
    var out = []
    var fixtures = st.prev_fixtures || []
    var squad = st.prev_squad || []
    for (var i = 0; i < fixtures.length; i++) {
      var f = fixtures[i]
      if (!Fmt.matches(f.label, q) && !Fmt.matches(f.home_name, q)
          && !Fmt.matches(f.away_name, q)) continue
      out.push(tab.matchRow(f, squad, []))
    }
    return out
  }
  ...
  readonly property var rows: {
    ...
    for (var i = 0; i < fixtures.length; i++) {
      ...
      if (!Fmt.matches(f.label, q) && !Fmt.matches(f.home_name, q)
          && !Fmt.matches(f.away_name, q)) continue
      out.push(tab.matchRow(f, squad, races))
    }

    var past = tab.pastRows
    if (past.length > 0 && st.prev_gw) {
      out.push({ kind: "fold", gw: st.prev_gw, count: past.length })
      if (tab.showPast)
        for (var m = 0; m < past.length; m++) out.push(past[m])
    }
    return out
  }
  ...
  Item {
    ...
    height: headingRow.implicitHeight + Style.spacing.sm
    ...
    Row {
      ...
      Text {
        ...
        anchors.baseline: standingText.baseline
        text: "Gameweek " + (tab.st.gw || "—")
      ...
      Text {
        ...
        id: standingText
        text: tab.standing
        color: (tab.st.live_matches || 0) > 0 ? (app ? app.accent : "#fff")
                                              : (app ? app.dim : "#aaa")
        font.family: app ? app.fontFamily : "monospace"
  ...
  ListView {
    ...
    // Two kinds of row share the list: the match cards, and the one bar that
    // opens last week underneath them. A loader keeps the card's bindings out
    // of existence on the row that has no match behind it.
    delegate: Item {
      id: slot
      ...
      Loader {
        ...
        sourceComponent: slot.modelData.kind === "fold" ? foldBar : matchCard
      }
      ...
      Component {
        ...
        Rectangle {
          implicitHeight: Math.max(Style.space(34), foldText.implicitHeight + Style.spacing.md * 2)
          height: implicitHeight
          radius: app ? app.cornerRadius : 0
          color: slot.current ? (app ? app.selectedBackground : "#222")
                              : Util.alpha(app ? app.foreground : "#fff", 0.05)
          border.width: 1
          border.color: app ? app.hairline : "#333"
          ...
          Text {
            ...
            text: (tab.showPast ? "▾  " : "▸  ") + "Gameweek " + slot.modelData.gw + " results"
            color: slot.current ? (app ? app.selectedText : "#fff")
                                : (app ? app.foreground : "#fff")
            font.family: app ? app.fontFamily : "monospace"
          ...
          Text {
            ...
            anchors.verticalCenter: parent.verticalCenter
            text: slot.modelData.count + (slot.modelData.count === 1 ? " match" : " matches")
                  + "  ·  " + (tab.showPast ? "Enter to fold away" : "Enter to open")
      ...
      Component {
        ...
        Rectangle {
          id: card
          readonly property var fx: slot.modelData.fixture
          ...
          MouseArea {
            anchors.fill: parent
            cursorShape: Qt.PointingHandCursor
            onClicked: if (app) app.selectedIndex = slot.index
          }
          ...
          Column {
            ...
            Item {
              ...
              Row {                                             // clock and stake
                ...
                Text {
                  textFormat: Text.PlainText
                  anchors.verticalCenter: parent.verticalCenter
                  text: slot.modelData.mine.length > 0
                          ? slot.modelData.mine.length + " of yours" : ""
                  color: app ? app.fainter : "#888"
                  font.family: app ? app.fontFamily : "monospace"
                  font.pixelSize: Style.font.caption
                }
            ...
            Item {
              ...
              visible: slot.modelData.mine.length > 0
            ...
            Item {
              ...
              visible: tab.statto ? !!card.fx.referee
                                  : (slot.modelData.race !== null && slot.modelData.race !== undefined)
              ...
              Column {
                ...
                Repeater {
                  model: (!tab.statto && slot.modelData.race) ? slot.modelData.race.rows : []
                  delegate: Row {
                    required property var modelData
                    spacing: Style.spacing.md
